...
import com.anylogic.ssystem.bms.dct.service.BmsDctEnfHistoryService;
...
@Api(tags = {"dct"})
@RestController
@RequestMapping("/bms/dct/bmsdctenfhistory")
public class BmsDctEnfHistoryController {

    @Autowired
    private BmsDctEnfHistoryService bmsDctEnfHistoryService;

    /* 접수 처리 */
    @PostMapping(value = "/documentProcBIA")
    public ResponseEntity documentProcBIA
    (
        @Valid @RequestBody DocumentProcBIAReqVO param
    )
    {
        log.debug("bmsDctEnfHistory documentProcBIA" + param.toString());

        int res = 0;
        try {
            res = bmsDctEnfHistoryService.documentProcBIA(param);
        } catch (Exception e) {

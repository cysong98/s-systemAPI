...
import com.anylogic.ssystem.bms.use.service.BmsDctApprreqLendService;
...
@Api(tags = {"use"})
@RestController
@RequestMapping("/bms/use/bmsdctapprreqlend")
public class BmsDctApprreqLendController {

    @Autowired
    private BmsDctApprreqLendService bmsDctApprreqLendService;


    /* 비밀문서사용요청 > 반출요청 > 전자 반출요청서 등록 */
    @PostMapping(value = "/insertUseTakeOut")
    public ResponseEntity insertUseTakeOut
    (
        @Valid @RequestBody InsertUseTakeOutReqVO param
    ) 
    {
        int res = 0;
        try {
            res = bmsDctApprreqLendService.insertUseTakeOut(param);
        } catch (Exception e) { 
    ...
    @PostMapping(value = "/insertUseNondigitalTakeOut")
    public ResponseEntity insertUseNondigitalTakeOut
    (
        @Valid @RequestBody InsertUseTakeOutReqVO param
    )
    {
        int res = 0;
        try {
            res = bmsDctApprreqLendService.insertUseNondigitalTakeOut(param);
        } catch (Exception e) {
            log.error(e.getMessage());
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }

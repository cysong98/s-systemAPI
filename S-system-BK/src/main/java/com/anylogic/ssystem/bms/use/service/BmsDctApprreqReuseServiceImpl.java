...
import com.anylogic.ssystem.bms.use.mapper.BmsDctApprreqReuseMapper;
import com.anylogic.ssystem.bms.use.model.InsertReUseReqReqVO;

import java.math.BigDecimal;
import java.text.SimpleDateFormat;
import java.util.*;
import com.anylogic.ssystem.bms.use.model.InsertDctApprReqReqVO;
...
@Service("BmsDctApprreqReuseService")
@Transactional
public class BmsDctApprreqReuseServiceImpl implements BmsDctApprreqReuseService {
    ...
    @Transactional
    @Override
    public int insertReUseReq(InsertReUseReqReqVO param){
        int resInsertReUseReq = 0;
        String resSysGuid = "";
        ...
        try {
            ...
            for(InsertReUseReqReqVO tempReqVO : param.getPathList()){
            ...
            // 03. INSERT BMS_DCT_APPRREQ_REUSE TABLE
            for(InsertReUseReqReqVO tempReqVO : param.getReuseList()) {
                String begintdt = tempReqVO.getReqreusebegindt().replaceAll("-","")+ "000000";
                String enddt = tempReqVO.getReqreuseenddt().replaceAll("-","")+ "235959";
                tempReqVO.setReqreusebegindt(begintdt);
                tempReqVO.setReqreuseenddt(enddt);
                tempReqVO.setApprreqid(param.getApprreqid());
                InsertReUseReqReqVO insertReUseReqParam = getApprreqReuseVO(tempReqVO);
                resInsertReUseReq += bmsDctApprreqReuseMapper.insertReUseReq(insertReUseReqParam);
            }

            // 04. INSERT BMS_DCT_HIST_CARD TABLE
            int resInsertHistory = 0;
            String histSeq = bmsDctHistCardMapper.getHistSeq();
            param.setHistgubun("07"); // 재사용
        ...
        return resInsertReUseReq;
    }

    // 01. INSERT BMS_DCT_APPRREQ TABLE
    private InsertDctApprReqReqVO getApprreqVO(InsertReUseReqReqVO vo, String indt){
    ...
    // 02. INSERT BMS_DCT_APPRREQ_PATH TABLE
    private InsertDctApprReqPathReqVO getApprreqPathVO(InsertReUseReqReqVO vo, String indt){
    ...
    // 03. INSERT BMS_DCT_APPRREQ_REUSE TABLE
    private InsertReUseReqReqVO getApprreqReuseVO(InsertReUseReqReqVO vo){
        InsertReUseReqReqVO build = InsertReUseReqReqVO.builder()
                .apprreqid(vo.getApprreqid())
                .mgmtid(vo.getMgmtid())
                .reqreusebegindt(vo.getReqreusebegindt())
                .grantreusebegindt(vo.getGrantreusebegindt())
                .grantreuseenddt(vo.getGrantreuseenddt())
                .reqreuseenddt(vo.getReqreuseenddt())
                .grantflag(vo.getGrantflag())
                .build();
        return build;
    }
    ...
    // 04. INSERT BMS_DCT_HIST_CARD TABLE
    private InsertHistoryReqVO getHistoryVO (InsertReUseReqReqVO vo, String resDocid, String histSeq, String indt){

...
public class HTTPUtil
{
    ...
    {
        //소문자로 모두 변환
        Map<String, String> checkHeaders = new HashMap<>();
        ...
        for(String defaultKey : defaultKeys)
        {
            ...
            if(value == null)
            {
                ...
                Objects.requireNonNull(headers).put(defaultKey, defaultValue);
            }
        }
    }
...
//                nowHttpURLConnection = httpsURLConnection;
     ...
     * @param url    : String 전송할 목적지(http)
     * @param sendData    : String 전송할 데이터
     * @param timeout : int time out 시간
     ...
     * HTTP 방식으로 데이터 전송
     * @param url    : String 전송할 목적지(http)
     * @param method    : String 방법
     * @param contentType    : String content Type
     * @param headers    : Map<String, String> 전송 header(http)
     * @param sendByte    : byte[] 전송 데이터
     * @param timeout    : int 전송 time out 시간
    ...
    public static String doAction(String url, String method, String contentType, Map<String, String> headers, byte[] sendByte, int timeout) throws Exception
    {
        ...
        if(httpResponse.getRpyPackt() == null)
        {
            return null;
        }

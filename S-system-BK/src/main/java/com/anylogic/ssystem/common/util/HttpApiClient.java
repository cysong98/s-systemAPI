...
public class HttpApiClient {
    ...
    public String sendApi(HttpMethod method ,String interfaceUrl, Map<String, Object> reqVO) {
        ...
        HttpEntity<Map<String, Object>> entitiry;
        
        if (!HttpMethod.DELETE.equals(method)) {
            entitiry = new HttpEntity<>(reqVO, headers);
        } else {
            entitiry = new HttpEntity<>(null, headers);
        }
        
        res = restTemplate.exchange(interfaceUrl, method, entitiry, byte[].class);
        
        resultData = new String(Objects.requireNonNull(res.getBody()));

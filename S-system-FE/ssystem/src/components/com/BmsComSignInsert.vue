  ...
  <div class="page" :style="{width: contentWidth}">
    ...
    <div class="box-content">
      <section class="section sticky">
        <div class="box-title">
          <h5 class="h5">서명인 등록</h5>
          <div class="action-buttons">
            <v-btn outlined @click="insertSignBIA()">저장</v-btn>
          ...
          <tbody>
            <tr>
              ...
              <td>
                <v-select
                  label="분류"
                  v-model="insertSignBIAData.managegubun"
                  :items="comboData"
                  item-title="view"
                  item-value="key"
                  variant="outlined"
            ...
            <tr>
              ...
              <td>
                <div>
                  <v-text-field :readonly="true" v-model="insertSignBIAData.indt" variant="outlined" hide-details="auto"></v-text-field>
                </div>
              </td>
            </tr>
          </tbody>
...
const name = ref('BmsComSignInsert')
...
const addFile = async (e) => {
  try {
    ...
    for (const file of files) {
      ...
      insertSignBIAData.value.fileInfo = [];
      insertSignBIAData.value.fileInfo.push(createdFile);
      previewImage(file);
    }
  } catch {
    return;
...
const insertSignBIA = async () => {
  if (checkRVComOrgDetailInfo() == false)
    return;
  isloading.value = true;

  // insertSignBIAData 입력 정보
  insertSignBIAData.value.indt = dayjs(new Date()).format('YYYY-MM-DD');
  if(insertSignBIAData.value.managegubun === "ORGEA")
    insertSignBIAData.value.etc = "서명인";
  else
    insertSignBIAData.value.etc = "생략인";

  // 이미 저장한 적이 있는지 체크
  checkAlreadySaved.value.orgid = insertSignBIAData.value.orgid;
  checkAlreadySaved.value.managegubun = insertSignBIAData.value.managegubun;
  const response = await API.comAPI.getSignDetail(checkAlreadySaved.value, urlPaths.value);
  
  if(response.data !== "") {
    alert(`${orgname.value}의 ${insertSignBIAData.value.etc}에는 이미 등록된 이미지가 존재합니다.`);
  ...
  // 추가된 것
  insertSignBIAData.value.indexComment = [];
  let fIdx = 0;

  for await(const f of insertSignBIAData.value.fileInfo) {
    insertSignBIAData.value.indexComment.push({
      fileIndex: fIdx++,
    });
  ...
  insertSignBIAFormData.append(
    "requiredInsertSignBIA",
    new Blob([JSON.stringify(insertSignBIAData.value)], {
  ...
  API.comAPI
    .insertSignBIA(insertSignBIAFormData, urlPaths.value)
    .then(response => {
...
const selectOrgList = ref(async (retValue) => {
  ...
  toggleOrg.value();
  insertSignBIAData.value.orgid = retValue.orgid;
...
const checkRVComOrgDetailInfo = () => {
  if(fileinfo.value.length === 0) {
    alert("서명인 이미지를 등록하세요");
    return false;
  }
  if (insertSignBIAData.value.orgid === ''){
    alert("부서를 선택해주세요.");
    return false;
  }

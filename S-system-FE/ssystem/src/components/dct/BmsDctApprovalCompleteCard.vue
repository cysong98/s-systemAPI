  ...
  <div class="page" :style="{width: contentWidth}">
    <div class="box-title">
      ...
      <div class="action-buttons3">
        ...
        <v-btn class="button" @click="dctManageRecordPopupToggleFunc">비밀관리기록부</v-btn>
      </div>
    </div>
      ...
      <section class="section sticky">
        <div class="box-title">
          ...
          <div class="action-buttons">
            ...
            <v-btn class="button" @click="moveToBmsDctApprovalCompletelist()">목록</v-btn>
          </div>
        </div>
      </section>
          ...
          <v-table :class="{ 'table-type-03-copydoc': isReceivedDoc, 'table-type-03': !isReceivedDoc }">
            ...
            <tbody>
              ...
              <tr v-if="!isReceivedDoc">
                <th>문서취지</th>
                <td colspan="3">
                  <span>{{ updateCardData.summary }}</span>
                </td>
              </tr>
              <tr>
                <th>기안문</th>
                <td colspan="3">
                  <div class="box-flex" v-if="updateCardData.draftdocttl">
                    ...
                    <span @click="readDraftDoc()" style="font-weight: 600; cursor: pointer;">{{ updateCardData.draftdocttl }}</span>
                  </div>
                </td>
              </tr>
              <tr>
...
const dctManageRecordPopupToggleFunc = ref(() => {
  if(updateCardData.value.mgmtno != "" && updateCardData.value.mgmtno != null && updateCardData.value.mgmtno != undefined) {
    bVisibleDctManageRecordPopup.value = !bVisibleDctManageRecordPopup.value;
  } else {
    alert("비밀관리기록부가 존재하지 않습니다.");
  }
})
...
const orgnReclassSelected = computed(() => {
  if(updateCardData.value.orgnreclass == '21'){
    return '일반문서';
  }else{
    return '선택'
  }
...
const copyReclassSelected = computed(() => {
  if(updateCardData.value.copyreclass == '21'){
    return '일반문서';
  }else{
    return '선택'
  }
...
const makeAlert = () => {
  ...
  if (updateCardData.value.orgnreclass == '') {
  ...
  if (updateCardData.value.copyreclass == '') {
    alert('사본 보호기간 조건을 선택해주세요.');
    return false;
  }
  if (parseInt(updateCardData.value.prsrvterm) < parseInt(orgnProtDtPeriodSelected.value)) {
    alert('예고문 보호기간은 원본 보호기간보다 작을 수 없습니다.');
    return false;
  }
  if (parseInt(updateCardData.value.prsrvterm) < parseInt(copyProtDtPeriodSelected.value)) {
...
onBeforeMount(() => {
  ...
  getMgmtidByDocidCondi.value.docid = docidData.value.docid;
  dctAuthManageCardPopupArgs.value.docid = docidData.value.docid;
  dctHistoryCardPopupArgs.value.docid = docidData.value.docid;
  dctDistributeCardPopupArgs.value.docid = docidData.value.docid;
...
const getDctRdoc = async () => {
  try {
    ...
    state.dctDistributeCardPopupArgs.stateCheck = updateCardData.value.state;
...
const selectDctPath = async () => {
  try {
    ...
    for (let i = 0; i < response.data.length; i++) {
      ...
      if (response.data[i].reportdt != "" && response.data[i].reportdt != null) {
        upd.reportdt = transformDate(response.data[i].reportdt);
      } else {
...
const selectDctFile = async () => {
  try {
    ...
    for (let j = 0; j < updateCardData.value.draftfile.length; j++) {
      indtArr.push(Number(updateCardData.value.draftfile[j].indt));
      if (updateCardData.value.authorid == updateCardData.value.draftfile[j].inid) {
        arrIdx.push(j);
      }
    }
    if (arrIdx.length > 1) {
      updateCardData.value.draftfile.splice(arrIdx[0], 1);
    }

    let maxIndt = Math.max(...indtArr);
    for (let k = 0; k < updateCardData.value.draftfile.length; k++) {
      if (Number(updateCardData.value.draftfile[k].indt) == Number(maxIndt)) {
        updateCardData.value.draftdocttl = updateCardData.value.draftfile[k].sfilename;
        updateCardData.value.drafturl = updateCardData.value.draftfile[k].fleurl;
      } else if (updateCardData.value.draftfile.length == 1) {
        updateCardData.value.draftdocttl = updateCardData.value.draftfile[0].sfilename;
        updateCardData.value.drafturl = updateCardData.value.draftfile[0].fleurl;
      }
    }
  } catch (error) {
    console.log(error);
...
const getMgmtidByDocid = async () => {
  try {
    ...
    dctManageRecordPopupArgs.value.docid = getMgmtidByDocidCondi.value.docid;
    dctManageRecordPopupArgs.value.mgmtid = getMgmtidByDocidList.value.mgmtid;
    dctHistoryCardPopupArgs.value.mgmtid = getMgmtidByDocidList.value.mgmtid;
  } catch (error) {
    console.log(error);
    alert("Server Error");
  }

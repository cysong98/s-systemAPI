      ...
      <section class="section sticky">
        <div class="box-title">
          ...
          <div class="action-buttons">
            <v-btn v-if="getActtype == '반려'" @click="rejectedDocumentToTempSave()">반려삭제</v-btn>
            ...
            <v-btn class="button" @click="moveToBmsDctReqCompletelist()">목록</v-btn>
          </div>
        </div>
      </section>
...
import { getValueByKey, getFileIcon, viewDocument, getExtension, url } from "@/utils/Utils.js";
import { API } from "@/api";
import {
  setQueries,
  setCondiChanged,
...
const name = ref('BmsDctreqcompletecard')
...
onBeforeMount(() => {
  ...
  getActtype.value = query.value.acttype;
  ...
  try {
    ...
    for(let i=0; i<response.data.length; i++){
      ...
      if(response.data[0].distdeptname != null){
        ...
        showCreateCardData.value.sendNameData = response.data[0].distname;
      }
    }
  } catch (error) {
    console.log(error);
...
// 취소 버튼 Move FN (기안완료함 목록 페이지로 이동)
const moveToBmsDctReqCompletelist = () => {
  router.push({
  ...
  if (showCreateCardData.value.draftfile.length > 0) {
    if (pathData != null && pathData != undefined) {
      ...
      for (let i = 0; i < showCreateCardData.value.pathList.length; i++) {
        ...
        for (let k = 0; k < showCreateCardData.value.draftfile.length; k++) {
          if (object.id == showCreateCardData.value.draftfile[k].inid) {
            object.url = showCreateCardData.value.draftfile[k].fleurl;
            break;
          } else {
            object.url = "";
          }
...
const rejectedDocumentToTempSave = async () => {
  ...
  const rejectedDocumentToTempSaveParam = [];
  rejectedDocumentToTempSaveParam.push({docid: docidData.value.docid})
  await API.dctAPI
  .rejectedDocumentToTempSave(rejectedDocumentToTempSaveParam, urlPaths.value)
  .then(response => {

...
<template>
  <div class="page" :style="{width: contentWidth}">
    ...
    <div class="box-content">
      ...
      <section class="section sticky">
        <div class="box-title">
          ...
          <div class="action-buttons">
            ...
            <!-- <v-btn class="button" @click="tempSave()">임시저장</v-btn> -->
            <v-btn class="button" @click="dctTempPopupToggleFunc()">임시저장</v-btn>
            <v-btn class="button" @click="moveToBmsDctTemplist()">목록</v-btn>
          </div>
        </div>
      </section>
          ...
          <tbody>
            ...
            <tr>
              <th>
                ...
                <!-- </div> -->
                <!-- <v-btn min-width="100px" v-if="updateCreateCardData.fileList.length >0" @click="downloadFileAll(updateCreateCardData.fileList)">일괄다운</v-btn> -->
              </th>
              <td colspan="3">
                <div>
                  <FilePond
        ...
        <v-table class="table-type-03">
          ...
          <tbody>
            <tr>
              ...
              <td>
                <v-text-field
                  v-model="updateCreateCardData.docno"
                  :readonly="true"
                  variant="solo"
                  hide-details="auto"></v-text-field>
              </td>
      ...
      <HwpCtrl v-if="draftToggle.data == 'Y'" :data="updateCreateCardData" :toggle="draftToggle" :view="draftView" :received="isReceivedDoc" ref="childRef" :key="compKey"/>
    </div>    
...
const dctReqProcPopupToggleFunc = ref(() => {
  ...
  //   childRef.value.saveFile(updateCreateCardData.value.docttl, 'Y');
  // }
...
const dctReferencePopupReturnFunc = ref((retValue) => {
  ...
  if(retValue.ref != ""){
    if(updateCreateCardData.value.refList[0] != undefined){
      // if(updateCreateCardData.value.refList[0].infotype == ""){
        updateCreateCardData.value.refList = [];
      // }
    }
...
onBeforeMount(() => {
  ...
  updateCreateCardData.value.resdocid = docidData.value.docid;
});
...
const delRecv = () => {
  if (chkbox.value) {
    updateCreateCardData.value.recvList = [];
    updateCreateCardData.value.sendOrgNameData = "";
    updateCreateCardData.value.sendNameData = "";
  }
};
...
const dctTempPopupToggleFunc = ref(() => {
  ...
  dctTempPopupArgs.value.data = updateCreateCardData.value;
...
const getDctRdoc = async () => {
  try {
    ...
    if(data.emergency !== "n"){
      updateCreateCardData.value.emergency = data.emergency;
    }
    updateCreateCardData.value.keyword = data.keyword ? data.keyword : '';
    updateCreateCardData.value.authorname = data.authorname;
    updateCreateCardData.value.authordutyname = data.authordutyname;
    updateCreateCardData.value.authordeptname = data.authordeptname;
    updateCreateCardData.value.tempflag = data.tempflag;
    updateCreateCardData.value.indt = data.indt;
    if(updateCreateCardData.value.gubun == 'DOPT1') {
      chkbox.value = true;
    } else {
      chkbox.value = false;
    }
...
const isReceivedDoc = ref(false);
 ...
 const getDctAttr = async () => {
  try {
    ...
    const data = response.data;
    updateCreateCardData.value.secgubun = data.secgubun;
    updateCreateCardData.value.seclevel = data.seclevel;
    updateCreateCardData.value.prsrvterm = data.prsrvterm ? (data.prsrvterm.trim() != "" ? data.prsrvterm : '') : '';
    updateCreateCardData.value.orgnprotdt = data.orgnprotdt;
    updateCreateCardData.value.orgnnextlevel = data.orgnnextlevel ;
    updateCreateCardData.value.orgnreclass = data.orgnreclass ? (data.orgnreclass.trim() != "" ? data.orgnreclass : '') : '';
    updateCreateCardData.value.copyprotdt = data.copyprotdt;
    updateCreateCardData.value.copynextlevel = data.copynextlevel;
    updateCreateCardData.value.copyreclass = data.copyreclass ? (data.copyreclass.trim() != "" ? data.copyreclass : '') : '';
    isReceivedDoc.value = updateCreateCardData.value.copyno != null && !updateCardData.value.copyno.includes("원") ? true : false;
    if(updateCreateCardData.value.orgnprotdt){
      sliceDates(updateCreateCardData.value.orgnprotdt, orgnDate.value);
      orgnProtDtPeriodSelected.value = (parseInt(orgnDate.value.year) - parseInt(updateCreateCardData.value.indt.slice(0,4))).toString();
    }
    if(updateCreateCardData.value.copyprotdt){
      sliceDates(updateCreateCardData.value.copyprotdt, copyDate.value);
      copyProtDtPeriodSelected.value = (parseInt(copyDate.value.year) - parseInt(updateCreateCardData.value.indt.slice(0,4))).toString();
    }
  } catch (error) {
    console.log(error);
    alert("Server Error");
 ...
 const selectDctPath  = async () => {
  try {
    ...
    for(let i=0; i<response.data.length; i++){
      ...
      updateCreateCardData.value.pathList.push(upd);
    }
  } catch (error) {
    console.log(error);
    alert("Server Error");
 ...
 const getDctRecv = async () => {
  try {
    ...
    for(let i=0; i<response.data.length; i++){
      ...
      updateCreateCardData.value.recvList.push(upd);
      if(response.data[0].distdeptname != null){
        updateCreateCardData.value.sendOrgNameData = response.data[0].distdeptname;
        updateCreateCardData.value.sendNameData = response.data[0].distname;
      }
    }
  } catch (error) {
    console.log(error);
 ...
 const selectDctReader = async () => {
  try {
    ...
    for(let i=0; i<response.data.length; i++){
      ...
      updateCreateCardData.value.rcvsList.push(upd);
    }
  } catch (error) {
    console.log(error);
    alert("Server Error");
...
const formatRcvsList = computed(() => {
 ...
 const selectTaskInfoTemp  = async () => {
  try {
    const response = await API.dctAPI.selectTaskInfoTemp ({
      ...docidData.value },
      urlPaths.value
    );
    updateCreateCardData.value.refList = [];
 ...
 const selectDctFile = async () => {
  try {
    ...
    // 붙임파일 전체 (이력용)
    updateCreateCardData.value.fileListAll = [];
    for (let i = 0; i < response.data.length; i++) {
      if (response.data[i].gubun == 'DFT02') { // 붙임파일만
        ...
        updateCreateCardData.value.fileListAll.push(upd);

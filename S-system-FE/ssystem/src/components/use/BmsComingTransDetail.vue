    ...
    <div class="box-content">
        ...
        <v-table class="table-type-03">
          ...
          <tbody>
            <tr>
              ...
              <td>
                <v-text-field
                  v-model="insertComingTransApproveBIAData.reqttl"
            ...
            <tr>
              ...
              <td>
                <v-text-field
                  v-model="insertComingTransApproveBIAData.reqcontents"
      ...
      <section class="section">
        <div class="box-title">
          ...
          <div class="action-buttons">
            <v-btn class="buttons" :disabled="isChiefMe" @click="usePathPopupToggleFunc()">결재선지정</v-btn>
          </div>
        </div>
  ...
  <v-dialog v-model="bVisibleUsePathPopup" style="width: 850px;">
    ...
    <v-card>
      ...
      <UseApprovalPopup
        ...
        :insertFunc="insertComingTransApproveBIA"
      ></UseApprovalPopup>
    </v-card>
  </v-dialog>
...
const name = ref('BmsComingTransDetail')
/* ======================== 결재선지정 팝업 ======================== */
...
const usePathPopupToggleFunc = ref(() => { 
  const pathList = [...insertComingTransApproveBIAData.value.pathList];
...
const usePathPopupReturnFunc = ref((retValue) => {
  ...
  if(retValue != ""){
    const filteredList = insertComingTransApproveBIAData.value.pathList.slice(1);
    ...
    if(deletedPath.length > 0){
      deletedPath.forEach(identifier=>{
        const indexToRemove = insertComingTransApproveBIAData.value.pathList.findIndex(path => path.identifier === identifier);
        if (indexToRemove !== -1) {
          insertComingTransApproveBIAData.value.pathList.splice(indexToRemove, 1);
        }
      })
    }
    for(let i = 0; i < retValue.length; i++){
      ...
      if (!existingIdentifiers.has(path.identifier)) {
        insertComingTransApproveBIAData.value.pathList.push(path);
      }
    }

    const retValueIdentifiers = retValue.map(item => item.identifier);
    insertComingTransApproveBIAData.value.pathList.sort((a, b) => {
...
const sortedPathList = computed(() => {
  ...
  // '요청', '검토', '승인'에 따라 재정렬
  insertComingTransApproveBIAData.value.pathList.forEach((path) => {
  ...
  insertComingTransApproveBIAData.value.pathList = requests.concat(reviewers, approvers);
  updateApprreqSeq();

  return insertComingTransApproveBIAData.value.pathList
});

const updateApprreqSeq = () => {
  insertComingTransApproveBIAData.value.pathList.forEach((path, index) => {
...
const useApprovalPopupToggleFunc = ref(() => { 
  ...
  if(!insertComingTransApproveBIAData.value.reqttl){
    alert("제목을 작성해주세요")
    return
  }

  if(!insertComingTransApproveBIAData.value.reqcontents){
    alert("사유를 작성해주세요")
    return
  }

  if(insertComingTransApproveBIAData.value.pathList.length == 1 && isChiefMe.value == false){
    alert("결재선지정을 해주세요.")
    return
  }

  if(!checkPathList2(insertComingTransApproveBIAData.value) && isChiefMe.value == false){
  ...
  if(isChiefMe.value == true){
    insertComingTransApproveBIAData.value.reqstatus = '6';
    insertComingTransApproveBIAData.value.grantflag = 'Y';
  }
  
  useApprovalPopupArgs.value = insertComingTransApproveBIAData.value;
...
// for insertComingTransApproveBIA
const insertComingTransApproveBIAData = ref({

  // 1) bms_dct_apprreq table
  apprreqid: "", // 결재요청 ID
  reqtype: "12", // 요청타입 (01: 열람, 02: 재사용, 03: 인쇄, 04: 대출, 05: 지출, 06: 재분류, 07: 파기, 08: 존안, 09: 이관, 10: 이관대기, 11: 이관연기, 12: 내부이관, 13: 비전자, 14: 오프라인열람, 15: 인쇄용프린터, 16: 추가발송)
  reqttl: "[내부이관요청서] 내부이관을 요청합니다.", // 요청제목
  reqcontents: "", // 요청사유
  reqid: "", // 요청자 ID
  reqname: "", // 요청자명
  reqdeptid: "", // 요청자 부서ID
...
onBeforeMount(() => {
  ...
  insertComingTransApproveBIAData.value.transList = mgmtidArray.map(mgmtid => ({
    apprreqid: "",
    mgmtid: mgmtid,
    transdt: "",
  }));
...
onMounted(async () => {
  ...
  insertComingTransApproveBIAData.value.reqid = getUserLoginData.value.userid;
  insertComingTransApproveBIAData.value.reqname = getUserLoginData.value.username;
  insertComingTransApproveBIAData.value.reqdeptid = getUserLoginData.value.deptid;
  insertComingTransApproveBIAData.value.reqdeptname = getUserLoginData.value.deptname;
  insertComingTransApproveBIAData.value.dutyname = getUserLoginData.value.grade;
  insertComingTransApproveBIAData.value.vgroupid = getUserLoginData.value.vgroupid;
  insertComingTransApproveBIAData.value.docid = getDctMgmtRegiAttrList.value.map(item => item.docid);
  await insertUsePathList2(insertComingTransApproveBIAData.value, selectFindManagerList.value, getUserLoginData.value);
  await getDefaultPathUsers2(insertComingTransApproveBIAData.value);
})
...
const getDctMgmtRegiAttrHeaders = [
  ...
  { key: "protectinfo", title: "보존&보호기간", width: "150px", sortable: true, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "orgninfo", title: "예고문", width: "150px", sortable: true, filterable: true, align: "center", divider: true, class: "custom-class" },
...
const getDctMgmtRegiAttr = async () => {
  ...
  try {
    ...
    response.data = response.data.map((u) => {
        ...
        u = { ...u, protectinfo: transformNotiprsrvterm(u.notiprsrvterm) +" & "+dayjs(u.notiprotterm, 'YYYYMMDDHHmmss').format('YYYY.MM.DD') };
        u = { ...u, orgninfo: dayjs(u.orgnprotdt, 'YYYYMMDD').format('YYYY.MM.DD') + transReclassInfo(u.orgnreclass)};
        return { ...u};
    });
...
const insertComingTransApproveBIA = async (param) => {
  isloading.value = true;
  param.pathList[0].appropinion = param.appropiniontemp;

  API.useAPI
    .insertComingTransApproveBIA(param, urlPaths.value)
    .then(response => {

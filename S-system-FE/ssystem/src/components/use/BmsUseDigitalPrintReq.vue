    ...
    <div class="box-content">
        ...
        <v-table class="table-type-03">
          ...
          <tbody>
            <tr>
              ...
              <td>
                <v-text-field
                  v-model="insertUseDigitalPrintData.reqttl"
            ...
            <tr>
              ...
              <td>
                <v-text-field
                  v-model="insertUseDigitalPrintData.reqcontents"
      ...
      <section class="section">
              ...
              <div v-for="file in item.raw.attachfileList" :key="file.id" >
                <ul class="attach-list ml-16" v-if="file.sfilename != undefined">
                  <li>
                    ...
                    <v-checkbox v-model="file.isSelected" hide-details class="flex-0-1 density"></v-checkbox>
        ...
        <template v-slot:item.reqprtcnt="{ item }">
          <v-text-field v-model="item.raw.reqprtcnt" :rules="[rules.prtCnt]" @update:modelValue="updateReqPrtCnt(item)" variant="outlined" hide-details="auto"/>
        </template>
  ...
  <v-dialog v-model="bVisibleUsePathPopup" style="width: 850px;">
    ...
    <v-card>
      ...
      <UseApprovalPopup
        ...
        :insertFunc="insertUseDigitalPrint"
      ></UseApprovalPopup>
    </v-card>
  </v-dialog>
...
const usePathPopupToggleFunc = ref(() => { 
  const pathList = [...insertUseDigitalPrintData.value.pathList];
...
const usePathPopupReturnFunc = ref((retValue) => {
  ...
  if (retValue !== "") {
    const filteredList = insertUseDigitalPrintData.value.pathList.slice(1);
    ...
    if(deletedPath.length > 0){
      deletedPath.forEach(identifier=>{
        const indexToRemove = insertUseDigitalPrintData.value.pathList.findIndex(path => path.identifier === identifier);
        if (indexToRemove !== -1) {
          insertUseDigitalPrintData.value.pathList.splice(indexToRemove, 1);
        }
      })
    }
    ...
    for(let i = 0; i < retValue.length; i++){
      ...
      if (!existingIdentifiers.has(path.identifier)) {
        insertUseDigitalPrintData.value.pathList.push(path);
      }
    }

    const retValueIdentifiers = retValue.map(item => item.identifier);
    insertUseDigitalPrintData.value.pathList.sort((a, b) => {
...
const sortedPathList = computed(() => {
  ...
  // '요청', '검토', '승인'에 따라 재정렬
  insertUseDigitalPrintData.value.pathList.forEach((path) => {
  ...
  insertUseDigitalPrintData.value.pathList = requests.concat(reviewers, approvers);
  updateApprreqSeq();

  return insertUseDigitalPrintData.value.pathList
});

const updateApprreqSeq = () => {
  insertUseDigitalPrintData.value.pathList.forEach((path, index) => {
...
const useApprovalPopupToggleFunc = ref(() => { 
  ...
  if(!insertUseDigitalPrintData.value.reqttl){
    alert("제목을 작성해주세요")
    return
  }

  if(!insertUseDigitalPrintData.value.reqcontents){
    alert("사유를 작성해주세요")
    return
  }

  if(insertUseDigitalPrintData.value.pathList.length == 1 && isChiefMe.value == false){
    alert("결재선지정을 해주세요.")
    return
  }

  if(!checkPathList(insertUseDigitalPrintData.value) && isChiefMe.value == false){
  ...
  let dateNull = 0;
  let prtNull = 0;
  insertUseDigitalPrintData.value.printList.forEach(item => {
  ...
  if(isChiefMe.value == true){
    insertUseDigitalPrintData.value.reqstatus = '6';
    insertUseDigitalPrintData.value.grantflag = 'Y';
    insertUseDigitalPrintData.value.printList.forEach(data => {
      data.grantprtcnt = data.reqprtcnt;
      data.grantdestructdt = data.reqdestructdt.replace(/-/g, '')+'235959';
      data.grantflag = 'Y';
    });
  }

  useApprovalPopupArgs.value = insertUseDigitalPrintData.value;
...
// for insertUseDigitalPrint
const insertUseDigitalPrintData = ref({
  ...
  printList: [
    {
      ...
      // 4) bms_dct_apprreq_prt_file table
      fileList: [
        {
          apprreqid: "", // 요청ID
          mgmtid: "", // 비밀관리 ID
...
onBeforeMount(() => {
  ...
  insertUseDigitalPrintData.value.printList = mgmtidArray.map(mgmtid => ({
    apprreqid: "", 
    mgmtid: mgmtid, 
    reqprtcnt: "",
    reqdestructdt: "",
...
onMounted(async () => {
  ...
  insertUseDigitalPrintData.value.reqid = getUserLoginData.value.userid;
  insertUseDigitalPrintData.value.reqname = getUserLoginData.value.username;
  insertUseDigitalPrintData.value.reqdeptid = getUserLoginData.value.deptid;
  insertUseDigitalPrintData.value.reqdeptname = getUserLoginData.value.deptname;
  today.value = dayjs().format('YYYY-MM-DD');
  insertUseDigitalPrintData.value.dutyname = getUserLoginData.value.grade;
  insertUseDigitalPrintData.value.docid = getDctMgmtRegiList.value.map(item => item.docid);
  await insertUsePathList(insertUseDigitalPrintData.value, getDctMgmtRegiList.value, getUserLoginData.value);
  await getDeptInfo(insertUseDigitalPrintData.value);
  await getDefaultPathUsers(insertUseDigitalPrintData.value);
})
...
const updateDestructDt = (item) => {
  const targetIndex = insertUseDigitalPrintData.value.printList.findIndex(i => i.mgmtid === item.key);
  if (targetIndex !== -1) {
    if(item.raw.readauthexpiredt < item.raw.reqdestructdt){
        alert(`파쇄예정일은 열람만료일(${item.raw.readauthexpiredt}) 보다 클 수 없습니다.`);
        item.raw.reqdestructdt = "";
    }else{
      insertUseDigitalPrintData.value.printList[targetIndex].reqdestructdt = item.raw.reqdestructdt;
    }
  }
};
...
const updateReqPrtCnt = (item) => {
  const targetIndex = insertUseDigitalPrintData.value.printList.findIndex(i => i.mgmtid === item.key);
  if (targetIndex !== -1) {
    insertUseDigitalPrintData.value.printList[targetIndex].reqprtcnt = item.raw.reqprtcnt;
  }
  if(rules.prtCnt(item.raw.reqprtcnt) == '3자리 이내의 정수만 입력하실 수 있습니다.'){
    item.raw.reqprtcnt = "";
  }
};
...
const getDctMgmtRegiHeaders = [
  ...
  { key: "reqdestructdt", title: "파쇄예정일", width: "150px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "reqprtcnt", title: "인쇄부수", width: "150px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
...
const getDctMgmtRegiPrint = async () => {
  ...
  try {
    ...
    getDctMgmtRegiList.value = response.data.map(item => {
      const addedItem = { ...item, reqdestructdt: '', reqprtcnt: '' };

      addedItem.attachfileList = addedItem.attachfileList
        .filter(file => file.gubun === 'DFT01')
        .concat(
          addedItem.attachfileList
...
const insertUseDigitalPrint = async (param) => {
  isloading.value = true;

  param.pathList[0].appropinion = param.appropiniontemp;

  API.useAPI
    .insertUseDigitalPrint(param, urlPaths.value)
    .then(response => {
...
const moveToBmsUsePrintReqList = () => {
  router.push({
    name: getFromPage.value || "BmsUsePrintReqList",
  });
};

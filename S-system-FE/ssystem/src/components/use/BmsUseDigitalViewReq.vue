      ...
      <section class="section sticky">
        <div class="box-title">
          ...
          <div class="action-buttons">
            ...
            <v-btn class="button" text-color="black" @click="moveToBmsUseViewReqList()">닫기</v-btn>
          </div>
        </div>
      </section>
        ...
        <v-table class="table-type-03">
          ...
          <tbody>
            <tr>
              ...
              <td>
                <v-text-field
                  v-model="insertUseDigitalViewReqData.reqttl"
            ...
            <tr>
              ...
              <td>
                <v-text-field
                  v-model="insertUseDigitalViewReqData.reqcontents"
  ...
  <v-dialog v-model="bVisibleUsePathPopup" style="width: 850px;">
    ...
    <v-card>
      ...
      <UseApprovalPopup
        ...
        :insertFunc="insertUseDigitalViewReq"
      ></UseApprovalPopup>
    </v-card>
  </v-dialog>
...
const usePathPopupToggleFunc = ref(() => { 
  const pathList = [...insertUseDigitalViewReqData.value.pathList];
...
const usePathPopupReturnFunc = ref((retValue) => {
  ...
  if (retValue !== "") {
    const filteredList = insertUseDigitalViewReqData.value.pathList.slice(1);
    ...
    if(deletedPath.length > 0){
      deletedPath.forEach(identifier=>{
        const indexToRemove = insertUseDigitalViewReqData.value.pathList.findIndex(path => path.identifier === identifier);
        if (indexToRemove !== -1) {
          insertUseDigitalViewReqData.value.pathList.splice(indexToRemove, 1);
        }
      })
    }
    ...
    for(let i = 0; i < retValue.length; i++){
      ...
      if (!existingIdentifiers.has(path.identifier)) {
        insertUseDigitalViewReqData.value.pathList.push(path);
      }
    }

    const retValueIdentifiers = retValue.map(item => item.identifier);
    insertUseDigitalViewReqData.value.pathList.sort((a, b) => {
...
const sortedPathList = computed(() => {
  ...
  // '요청', '검토', '승인'에 따라 재정렬
  insertUseDigitalViewReqData.value.pathList.forEach((path) => {
  ...
  insertUseDigitalViewReqData.value.pathList = requests.concat(reviewers, approvers);
  updateApprreqSeq();

  return insertUseDigitalViewReqData.value.pathList
});

const updateApprreqSeq = () => {
  insertUseDigitalViewReqData.value.pathList.forEach((path, index) => {
...
const useApprovalPopupToggleFunc = ref(() => { 
  ...
  if(!insertUseDigitalViewReqData.value.reqttl){
    alert("제목을 작성해주세요")
    return
  }

  if(!insertUseDigitalViewReqData.value.reqcontents){
    alert("사유를 작성해주세요")
    return
  }

  if(insertUseDigitalViewReqData.value.pathList.length == 1 && isChiefMe.value == false){
    alert("결재선지정을 해주세요.")
    return
  }

  if(!checkPathList(insertUseDigitalViewReqData.value) && isChiefMe.value == false){
    alert("요청내용에 속한 부서의 비밀관리자/부서장 중 승인자가 존재해야 합니다. \n비밀관리자 또는 부서장을 추가하여주십시오.")
    return;
  }

  let dateNull = 0;
  insertUseDigitalViewReqData.value.readList.forEach(item => {
  ...
  if(isChiefMe.value == true){
    insertUseDigitalViewReqData.value.reqstatus = '6';
    insertUseDigitalViewReqData.value.grantflag = 'Y';
    insertUseDigitalViewReqData.value.readList.forEach(data => {
  ...
  useApprovalPopupArgs.value = insertUseDigitalViewReqData.value;
...
// for insertUseDigitalViewReq
const insertUseDigitalViewReqData = ref({
  ...
  pathList: [
    {
      ...
      apprstatus: "", // 결재요청상태 (APP00: - , APP10: 결재대개, APP20: 결재완료, APP30: 1인결재)
      apprid: "", // 결재자 ID
      apprname: "", // 결재자명
      apprdeptid: "", // 결재자 부서ID
...
onBeforeMount(() => {
  ...
  insertUseDigitalViewReqData.value.readList = mgmtidArray.map(mgmtid => ({
    apprreqid: "",
    mgmtid: mgmtid,
    reqreadbegindt: "",
    reqreadenddt: "",
...
onMounted(async () => {
  ...
  insertUseDigitalViewReqData.value.reqid = getUserLoginData.value.userid;
  insertUseDigitalViewReqData.value.reqname = getUserLoginData.value.username;
  insertUseDigitalViewReqData.value.reqdeptid = getUserLoginData.value.deptid;
  insertUseDigitalViewReqData.value.reqdeptname = getUserLoginData.value.deptname;
  today.value = dayjs().format('YYYY-MM-DD');
  insertUseDigitalViewReqData.value.dutyname = getUserLoginData.value.grade;
  insertUseDigitalViewReqData.value.docid = getDctMgmtRegiList.value.map(item => item.docid);
  await insertUsePathList(insertUseDigitalViewReqData.value, getDctMgmtRegiList.value, getUserLoginData.value);
  await getDeptInfo(insertUseDigitalViewReqData.value);
  await getDefaultPathUsers(insertUseDigitalViewReqData.value);
})
...
const updateReadList = (item, field) => {
  const targetIndex = insertUseDigitalViewReqData.value.readList.findIndex(i => i.mgmtid === item.key);
  if (targetIndex !== -1) {
    if(field=="reqreadbegindt"){
      insertUseDigitalViewReqData.value.readList[targetIndex].reqreadbegindt = item.raw.reqreadbegindt;
    }else if(field=="reqreadenddt"){
      insertUseDigitalViewReqData.value.readList[targetIndex].reqreadenddt = item.raw.reqreadenddt;
    }
  }
};
...
const getDctMgmtRegi = async () => {
  ...
  try {
    ...
    getDctMgmtRegiList.value.forEach(item => {
      const matchingReq = insertUseDigitalViewReqData.value.readList.find(req => req.mgmtid === item.mgmtid);
      if (matchingReq) {
...
const insertUseDigitalViewReq = async (param) => {
  isloading.value = true;

  param.pathList[0].appropinion = param.appropiniontemp;

  API.useAPI
    .insertUseDigitalViewReq(param, urlPaths.value)
    .then(response => {
...
const moveToBmsUseViewReqList = () => {
  router.push({
    name: getFromPage.value || "BmsUseViewReqList",
  });
};

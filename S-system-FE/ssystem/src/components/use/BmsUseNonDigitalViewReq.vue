  ...
  <div class="page" :style="{width: contentWidth}">
    ...
    <div class="box-content">
      <section class="section sticky">
        <div class="box-title">
          <h5 class="h5">비전자 열람 요청서</h5>
          <div class="action-buttons">
            <v-btn class="btn" text-color="black" :disabled="isChiefMe" @click="usePathPopupToggleFunc()">결재선지정</v-btn>
            <v-btn class="btn" text-color="black" @click="useApprovalPopupToggleFunc()">{{ isChiefMe ? '승인' : '승인요청' }}</v-btn>
            <v-btn class="btn" text-color="black" @click="moveToBmsUseViewReqList()">닫기</v-btn>
          </div>
        </div>
      </section>
        ...
        <v-table class="table-type-03">
          ...
          <tbody>
            <tr>
              ...
              <td>
                <v-text-field
                  v-model="insertUseNonDigitalViewReqData.reqttl"
            ...
            <tr>
              ...
              <td>
                <v-text-field 
                  v-model="insertUseNonDigitalViewReqData.reqcontents"
      ...
      <section class="section">
        ...
        <v-table class="table-type-02">
          ...
          <tbody>
            <tr v-for="manager, idx in insertUseNonDigitalViewReqData.managertemp" :key="idx">
              <td>
                <v-text-field
                  v-model="manager.apprtemp"
                  :readonly="true"
                  variant="outlined"
                  hide-details="auto"
                ></v-text-field>
      ...
      <section class="section">
        ...
        <template v-slot:expanded-row="{ columns, item }">
          ...
          <template v-slot:item.reqdate="{ item }">
            <div class="box-flex" style="margin-left: 70px;">
              <v-text-field type="date" :min="today" v-model="item.raw.reqreadbegindt" @update:modelValue="updateReadList(item, 'reqreadbegindt')" variant="outlined" hide-details="auto"/>
              ~
              <v-text-field type="date" :min="item.raw.reqreadbegindt" v-model="item.raw.reqreadenddt" @update:modelValue="updateReadList(item, 'reqreadenddt')" variant="outlined" hide-details="auto"/>
          </div>
          </template>
  ...
  <v-dialog v-model="bVisibleUsePathPopup" style="width: 850px;">
    ...
    <v-card>
      ...
      <UseApprovalPopup
        ...
        :insertFunc="insertUseNonDigitalViewReq"
      ></UseApprovalPopup>
    </v-card>
  </v-dialog>
...
const name = ref('BmsUseDigitalViewReq')
/* ======================== 관리자지정 팝업 ======================== */
...
const usePathPopupToggleFunc = ref(() => { 
  const pathList = [...insertUseNonDigitalViewReqData.value.pathList];
...
const usePathPopupReturnFunc = ref((retValue) => {
  ...
  if (retValue !== "") {
    const filteredList = insertUseNonDigitalViewReqData.value.pathList.slice(1);
    ...
    if(deletedPath.length > 0){
      deletedPath.forEach(identifier=>{
        const indexToRemove = insertUseNonDigitalViewReqData.value.pathList.findIndex(path => path.identifier === identifier);
        if (indexToRemove !== -1) {
          insertUseNonDigitalViewReqData.value.pathList.splice(indexToRemove, 1);
        }
      })
    }
    ...
    for(let i = 0; i < retValue.length; i++){
      ...
      if (!existingIdentifiers.has(path.identifier)) {
        insertUseNonDigitalViewReqData.value.pathList.push(path);
      }
    }

    const retValueIdentifiers = retValue.map(item => item.identifier);
    insertUseNonDigitalViewReqData.value.pathList.sort((a, b) => {
...
const sortedPathList = computed(() => {
  ...
  // '요청', '검토', '승인'에 따라 재정렬
  insertUseNonDigitalViewReqData.value.pathList.forEach((path) => {
  ...
  insertUseNonDigitalViewReqData.value.pathList = requests.concat(reviewers, approvers);
  updateApprreqSeq();

  return insertUseNonDigitalViewReqData.value.pathList
});

const updateApprreqSeq = () => {
  insertUseNonDigitalViewReqData.value.pathList.forEach((path, index) => {
...
const useApprovalPopupToggleFunc = ref(() => { 
  ...
  if(!insertUseNonDigitalViewReqData.value.reqttl){
    alert("제목을 작성해주세요")
    return
  }

  if(!insertUseNonDigitalViewReqData.value.reqcontents){
    alert("사유를 작성해주세요")
    return
  }

  if(insertUseNonDigitalViewReqData.value.pathList.length == 1 && isChiefMe.value == false){
    alert("결재선지정을 해주세요.")
    return
  }

  if(!checkPathList(insertUseNonDigitalViewReqData.value) && isChiefMe.value == false){
    alert("요청내용에 속한 부서의 비밀관리자/부서장 중 승인자가 존재해야 합니다. \n비밀관리자 또는 부서장을 추가하여주십시오.")
    return;
  }

  let dateNull = 0;
  insertUseNonDigitalViewReqData.value.readList.forEach(item => {
    if (!item.reqreadbegindt || !item.reqreadenddt) {
      dateNull++;
      return;
    }
  });
  if(dateNull>0){
    alert("요청열람기간을 선택해주세요")
    return
  }
  ...
  if(isChiefMe.value == true){
    insertUseNonDigitalViewReqData.value.reqstatus = '6';
    insertUseNonDigitalViewReqData.value.grantflag = 'Y';
    insertUseNonDigitalViewReqData.value.readList.forEach(data => {
      data.grantreadbegindt = data.reqreadbegindt.replace(/-/g, '')+'000000';
      data.grantreadenddt = data.reqreadenddt.replace(/-/g, '')+'235959';
      data.grantflag = 'Y';
    });
  }

  useApprovalPopupArgs.value = insertUseNonDigitalViewReqData.value;
...
// for insertUseNonDigitalViewReq
const insertUseNonDigitalViewReqData = ref({

  // 1) bms_dct_apprreq table
  apprreqid: "", // 결재요청 ID
  reqtype: "01", // 요청타입 (01: 열람, 02: 재사용, 03: 인쇄, 04: 대출, 05: 지출, 06: 재분류, 07: 파기, 08: 존안, 09: 이관, 10: 이관대기, 11: 인관연기, 12: 내부이관, 13: 비전자, 14: 오프라인열람, 15: 인쇄용프린터, 16: 추가발송)
  reqttl: "[비전자열람요청서] 열람을 요청합니다.", // 요청제목
  reqcontents: "", // 요청사유
  reqid: "", // 요청자 ID
  reqname: "", // 요청자명
  reqdeptid: "", // 요청자 부서ID
  ...
  // 3) bms_dct_apprreq_read table
  readList: [
    {
      apprreqid: "", // 요청ID
      mgmtid: "", // 비밀관리 ID
      reqreadbegindt: "", // 요청열람 시작일시
      reqreadenddt: "", // 요청열람 만료일시
      grantflag: "N", // 승인여부
      readcharge: "", // 기관보안관리자
    }
  ],
...
onBeforeMount(() => {
  ...
  insertUseNonDigitalViewReqData.value.readList = mgmtidArray.map(mgmtid => ({
    apprreqid: "",
    mgmtid: mgmtid,
    reqreadbegindt: "",
    reqreadenddt: "",
...
onMounted(async () => {
  ...
  insertUseNonDigitalViewReqData.value.reqid = getUserLoginData.value.userid;
  insertUseNonDigitalViewReqData.value.reqname = getUserLoginData.value.username;
  insertUseNonDigitalViewReqData.value.reqdeptid = getUserLoginData.value.deptid;
  insertUseNonDigitalViewReqData.value.reqdeptname = getUserLoginData.value.deptname;
  // await getDraftManager();
  today.value = dayjs().format('YYYY-MM-DD');
  insertUseNonDigitalViewReqData.value.dutyname = getUserLoginData.value.grade;
  insertUseNonDigitalViewReqData.value.docid = getDctMgmtRegiList.value.map(item => item.docid);
  await insertUsePathList(insertUseNonDigitalViewReqData.value, getDctMgmtRegiList.value, getUserLoginData.value);
  await getDeptInfo(insertUseNonDigitalViewReqData.value);
  await getDefaultPathUsers(insertUseNonDigitalViewReqData.value);
})
...
const updateReadList = (item, field) => {
  const targetIndex = insertUseNonDigitalViewReqData.value.readList.findIndex(i => i.mgmtid === item.key);
  if (targetIndex !== -1) {
    if(field=="reqreadbegindt"){
      insertUseNonDigitalViewReqData.value.readList[targetIndex].reqreadbegindt = item.raw.reqreadbegindt;
    }else if(field=="reqreadenddt"){
      insertUseNonDigitalViewReqData.value.readList[targetIndex].reqreadenddt = item.raw.reqreadenddt;
    }
  }
};
...
const getDctMgmtRegiHeaders = [
  ...
  { key: "reqdate", title: "요청열람기간", width: "150px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
...
const getDctMgmtRegi = async () => {
  ...
  try {
    ...
    getDctMgmtRegiList.value = response.data.map(item => ({
      ...item, reqreadbegindt: dayjs().format('YYYY-MM-DD'), reqreadenddt: '' 
    }));
    expandedItems.value = getDctMgmtRegiList.value.map(item => item.mgmtid);

    getDctMgmtRegiList.value.forEach(item => {
      const matchingReq = insertUseNonDigitalViewReqData.value.readList.find(req => req.mgmtid === item.mgmtid);
      if (matchingReq) {
        matchingReq.reqreadbegindt = item.reqreadbegindt;
      }
    });
  } catch (error) {
    console.log(error);
...
// const getDraftManager = async() => {
//   insertUseNonDigitalViewReqData.value.managertemp = [];
//   const addedAuthorIds = new Set();

//   getDctMgmtRegiList.value.forEach(data=>{
//     if (!addedAuthorIds.has(data.authorid)) {
//       const manager = {
//         apprreqseq: 0,
//         apprtemp: "최종승인",
//         apprtype: "3",
...
//       }
//       manager.apprid = data.authorid;
//       manager.apprname = data.authorname;
//       manager.apprdeptid = data.deptid;
//       manager.apprdeptname = data.deptname;
//       manager.apprdutyname = data.authordutyname;
//       manager.apprnametemp = data.authordutyname+"/"+data.authorname;
//       insertUseNonDigitalViewReqData.value.managertemp.push(manager);
//       addedAuthorIds.add(data.authorid);
//     }
//   })
// }


const insertUseNonDigitalViewReq = async (param) => {
  isloading.value = true;

  param.pathList[0].appropinion = param.appropiniontemp;
  param.readList.map(item=>item.readcharge = param.managertemp.id);

  API.useAPI
    .insertUseNonDigitalViewReq(param, urlPaths.value)
    .then(response => {

    ...
    <div class="box-content">
      <section class="section sticky">
        <div class="box-title">
          ...
          <div class="action-buttons" v-if="selectUseReqViewDetailCondi.frompage == 'BmsUsecompletelist'">
            <!-- 요청서완료처리 -->
            <v-btn class="button" @click="useReqStatusSave('3')">존안실행</v-btn>
            <v-btn class="button" @click="moveToBmsUsereqlist()">목록</v-btn>
          </div>      
        </div>
      </section>
      ...
      <section class="section">
        ...
        <v-data-table
          ...
          class="table-type-04"
          >
          <template v-slot:item.grantcnsrvdt="{ item }">
            <div class="box-flex jc-center" v-if="selectUseReqViewDetailCondi.frompage == 'BmsUsereqlist'">
              <v-text-field type="date" :min="today" v-model="item.raw.grantcnsrvdt" @update:modelValue="updateGrantCnsrvDate(item)" variant="outlined" hide-details="auto" />
            </div>
            <div class="box-flex jc-center" v-else>
              <v-text-field :readonly="true" v-model="item.raw.grantcnsrvdt" variant="outlined" hide-details="auto" />
            </div>
          </template>
...
const bmsUseConfirmPopupToggleFunc = ref((val) => { 
  if (selectUseReqViewDetailViewList.value.some(item => item.grantcnsrvdt === '')) {
    alert("승인존안기간 설정을 완료해주세요")
    return
  }
...
const updateGrantCnsrvDate = (item) => {
  const max = dayjs(item.raw.maxcnsrvdate, 'YYYY.MM.DD').format('YYYY-MM-DD');
  const grant = item.raw.grantcnsrvdt
  if(max < grant){
    alert(`승인존안기간은 최대존안가능기간(${item.raw.maxcnsrvdate}) 보다 클 수 없습니다.`);
    item.raw.grantcnsrvdt = max;
  }
}
...
const staticColumnsSelectUseReqViewList = [
  { key: "mgmtno", title: "관리번호", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "secttl", title: "제목", width: "300px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "authorname", title: "담당자", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "indt", title: "등록일자", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "copyno", title: "사본번호", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "maxcnsrvdate", title: "최대존안가능기간", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "reqmaxcnsrvdate", title: "요청존안기간", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "grantcnsrvdt", title: "승인존안기간", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "regirecvtype", title: "구분", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
];
...
const useReqStatusSave = async (status) => {
  ...
  if(status == '4' || status == '5'){ // 권한회수, 요청서회수
    ...
    updateUseReqViewAppStatusBIAData.value.savetype = 'cnsrvcomplete'
  }

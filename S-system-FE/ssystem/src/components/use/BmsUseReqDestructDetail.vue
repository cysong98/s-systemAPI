      ...
      <section class="section sticky">
        <div class="box-title">
          ...
          <div class="action-buttons" v-if="selectUseReqViewDetailCondi.frompage == 'BmsUsecompletelist'">
            <!-- 요청서완료처리 -->
            <v-btn class="button" @click="useReqStatusSave('3')">파기실행</v-btn>
            <v-btn class="button" @click="moveToBmsUsereqlist()">목록</v-btn>
          </div>      
        </div>
      </section>
...
const staticColumnsSelectUseReqViewList = [
  ...
  { key: "rgstdt", title: "등록일자", width: "100px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
...
const selectUseReqViewDetail = async () => {
  ...
  try {
    ...
    selectUseReqViewDetailViewList.value = response.data.mapList.map((u) => {
      u = { ...u, rgstdt: dayjs(u.rgstdt, 'YYYYMMDDHHmmss').format('YYYY.MM.DD') };
...
const useReqStatusSave = async (status) => {
  ...
  if(status == '4' || status == '5'){ // 권한회수, 요청서회수
    ...
    updateUseReqViewAppStatusBIAData.value.savetype = 'destructcomplete'
  }

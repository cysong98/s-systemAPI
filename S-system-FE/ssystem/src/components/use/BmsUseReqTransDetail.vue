  ...
  <div class="page" :style="{width: contentWidth}">
    ...
    <div class="box-content">
      <section class="section sticky">
        <div class="box-title">
          <h5 class="h5" v-if="selectUseReqViewDetailData.reqtype == '11'">이관연기요청서</h5>
          <h5 class="h5" v-else>내부이관요청서</h5>
          ...
          <div class="action-buttons" v-if="selectUseReqViewDetailCondi.frompage == 'BmsUsecompletelist'">
             <!--요청서완료처리-->
            <v-btn class="button" @click="useReqStatusSave('3')" v-if="selectUseReqViewDetailCondi.reqtype == '11'">이관연기실행</v-btn>
            <v-btn class="button" @click="useReqStatusSave('3')" v-if="selectUseReqViewDetailCondi.reqtype == '12'">내부이관실행</v-btn>
            <v-btn class="button" @click="moveToBmsUsereqlist()">목록</v-btn>
          </div>
        </div>
      </section>
      ...
      <section class="section">
        ...
        <!-- 이관연기 -->
        <v-data-table
          v-if="selectUseReqViewDetailCondi.reqtype == '11'"
          :headers="staticColumnsSelectUseReqViewListDelay"
          :items="selectUseReqViewDetailViewList"
          item-value="mgmtid"
          class="table-type-04"
          >
          <template v-slot:item.granttransextdt="{ item }">
            <v-select 
              v-if="selectUseReqViewDetailCondi.frompage == 'BmsUsereqlist'"
              v-model="item.raw.granttransextdt"
          ...
          <!-- 내부이관 -->
          <v-data-table
            v-if="selectUseReqViewDetailCondi.reqtype == '12'"
...
const { contentWidth, topMenu, breadcrumbs } = storeToRefs(mainStore)
  ...
  const name = ref('BmsUseReqDestructDetail')
/* ======================== 승인/반려 팝업 ======================== */
...
// 이관연기
const staticColumnsSelectUseReqViewListDelay = [
  ...
  { key: "notiprsrvtermnm", title: "보존기간", width: "100px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "notiprottermnm", title: "보호기간", width: "100px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "granttransextdt", title: "이관희망년도", width: "150px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
...
const staticColumnsSelectUseReqViewList = [
  ...
  { key: "authorname", title: "담당자", width: "100px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "indt", title: "등록일자", width: "100px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "protectinfo", title: "보존&보호기간", width: "150px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "orgninfo", title: "예고문", width: "150px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "regirecvtype", title: "구분", width: "100px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
];
...
const selectUseReqViewDetail = async () => {
  ...
  try {
    ...
    response.data.mapList = response.data.mapList.map((u) => {
        u = { ...u, protectinfo: u.notiprsrvtermnm +" & "+u.notiprottermnm };
...
const useReqStatusSave = async (status) => {
  ...
  if(status == '4' && selectUseReqViewDetailCondi.value.reqstatusnm === '실행완료' && selectUseReqViewDetailCondi.value.reqtype == '12'){
    alert("이미 이관되어 권한회수를 하실 수 없습니다.");
    return;
  }
  ...
  if(status == '4' || status == '5'){ // 권한회수, 요청서회수
    updateUseReqViewAppStatusBIAData.value.savetype = 'MGT62'; // 이관대기상태로 전환
    updateUseReqViewAppStatusBIAData.value.list = selectUseReqViewDetailViewList.value;
  }else if(status == '3'){
    if(selectUseReqViewDetailCondi.value.reqtype == '11'){ // 이관연기 실행
      updateUseReqViewAppStatusBIAData.value.savetype = 'transwaitcomplete';
      updateUseReqViewAppStatusBIAData.value.list = selectUseReqViewDetailViewList.value;
    }else if(selectUseReqViewDetailCondi.value.reqtype == '12'){ // 내부이관 실행
      updateUseReqViewAppStatusBIAData.value.savetype = 'transcomplete';
      updateUseReqViewAppStatusBIAData.value.list = selectUseReqViewDetailViewList.value;
    }
  }

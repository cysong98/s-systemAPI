      ...
      <section class="section sticky">
        <div class="box-title">
          ...
          <div class="action-buttons" v-if="selectUseReqViewDetailCondi.frompage == 'BmsUsecompletelist'">
            <!-- 요청서완료처리 -->
            <v-btn class="button" @click="useReqStatusSave('3')">열람실행</v-btn>
            <v-btn class="button" @click="moveToBmsUsereqlist()">목록</v-btn>
          </div>  
        </div>
      </section>
        ...
        <v-data-table
          ...
          <template v-slot:item.reqdate="{ item }">
            <div class="box-flex jc-center" v-if="selectUseReqViewDetailCondi.frompage == 'BmsUsereqlist'">
              <v-text-field type="date" :min="item.raw.reqreadbegindt" v-model="item.raw.grantreadbegindt" variant="outlined" hide-details="auto" />
              ~
              <v-text-field type="date" :min="item.raw.grantreadbegindt" v-model="item.raw.grantreadenddt" variant="outlined" hide-details="auto" />
            </div>
            <div class="box-flex jc-center" v-else>
              <!-- <v-text-field :readonly="true" v-model="item.raw.grantreadbegindt" variant="outlined" hide-details="auto" /> -->
              {{ item.raw.grantreadbegindt }}
              ~
              {{ item.raw.grantreadenddt }}
              <!-- <v-text-field :readonly="true" v-model="item.raw.grantreadenddt" variant="outlined" hide-details="auto" /> -->
            </div>
          </template>
...
const name = ref('BmsUseReqViewDetail')
/* ======================== 승인/반려 팝업 ======================== */
...
const bmsUseConfirmPopupToggleFunc = ref((val) => { 
  if (selectUseReqViewDetailViewList.value.some(item => item.grantreadbegindt === '' || item.grantreadenddt === '' || item.grantreadbegindt > item.grantreadenddt)) {
    alert("승인열람기간 설정을 완료해주세요")
    return
  }
...
const staticColumnsSelectUseReqViewList = [
  ...
  { key: "reqreadtran", title: "요청열람기간", width: "200px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },
  { key: "reqdate", title: "승인열람기간", width: "200px", sortable: false, filterable: true, align: "center", divider: true, class: "custom-class" },

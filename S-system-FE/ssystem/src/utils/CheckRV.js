...
const checkUtil = {
  ...
  checkSeianPwd: (id, password) => {
    // const checkreg = new RegExp(!/^[a-zA-Z0-9{}[]/?.,;:|)*~`!^-_+<>@#$%&\\=('\"]{8,12}$/);
    const checkreg = !/^[a-zA-Z0-9{}[]\/?.,;:||\)*~`!^-_+<>@#$%&\\=\('\"]{8,12}$/
    if (
      checkreg.test(password)
    ) {
    ...
    const checkChar = password.search(/[{}[]\/?.,;:||\)*~`!^-_+<>@#$%&\\=\('\"]/gi);
    ...
    // const repeatReg = new RegExp("/(\w)\1\1\1/")
    const repeatReg = /(\w)\1\1\1/
    if (repeatReg.test(password)) {
      // 같은 문자 3번 반복 금지
  ...
  checkPW: (str = "") => {
    // 번호
    const regExp = new RegExp(/^[a-zA-Z0-9{}[]\/?\/.\/,;:||\)*~`!^-_+<>@#$%&\\=\('\"]{6,12}$/);
    if (!regExp.test(str)) {
      return false;
    }
    return true;
  ...
  checkHpNo: (str = "") => {
    let testNumber = String(str); // 문자로 변환
    testNumber = testNumber.replace(/-/gi, "");

    // let regExp =
    // /^01([0|1|6|7|8|9]?)-?([0-9]{3,4})-?([0-9]{4})$/;
    const regExp = new RegExp(/^01([016789]{1})([0-9]{7,8})$/);
  ...
  checkTelNo: (str = "") => {
    ...
    if (testNumber.includes('-')) {
      const regExp = new RegExp(/^0[2-9]{1,2}-[0-9]{3,4}-[0-9]{4}$/);
      if (!regExp.test(testNumber)) {
        return false;
      }
    }
  ...
  checkJumin: (num = "") => {
    ...
    const isSSN = (jumin1, jumin2) => {
      let n = 2;
      let sum = 0;
      for (let i = 0; i < jumin1.length; i++) {
        sum += parseInt(jumin1.substr(i, 1)) * n++;
      }
      for (let j = 0; j < jumin2.length - 1; j++) {
    ...
    let socialRegNumber = String(num); // 문자로 변환
  ...
  checkRegNo: (str = "") => {
    ...
    let regNumber = String(str); // 문자로 변환
    regNumber = regNumber.replace(/-/gi, "");

    let arr_regno = regNumber.split("");
  ...
  // 사업자 번호 체크
  checkBizNo: (num = "") => {
    let testNumber = String(num); // 문자로 변환
    testNumber = testNumber.replace(/-/gi, "");
  ...
  checkId: (str = "") => {
    const idRegx = new RegExp(/^[a-zA-Z]+[a-zA-Z0-9]{5,12}$/);
    return idRegx.test(str);
  },

  // 이메일 주소 검증
  checkEmail: (str = "") => {
    const emailRegx = new RegExp(/^[-A-Za-z0-9_]+[-A-Za-z0-9_.]*[@]{1}[-A-Za-z0-9_]+[-A-Za-z0-9_.]*[.]{1}[A-Za-z]{2,5}$/);
    return emailRegx.test(str);
  },

  // IP 주소 검증
  checkIpAddr: (str = "") => {
    const number = new RegExp(/^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/);
    if (!number.test(str)) {
      return false;
    }
    return true;
  ...
  checkKor: (str = "") => {
    /* let kor =/^[0-9a-zA-Z]*$/; */
    const kor = new RegExp(/[a-z0-9]|[ []{}()<>?|`~!@#$%^&*-_+=,.;:\"'\\]/g);
    if (kor.test(str)) {
      return false;
    }
    return true;
